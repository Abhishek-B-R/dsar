...
- API lifecycle E2E coverage exists in `packages/backend/test/e2e/full-flow.api.e2e.test.ts`. Those tests use the stub harness (in-memory persistence, stub adapters).
- Real SQLite file plus filesystem storage coverage exists in `packages/backend/test/e2e/sqlite-filesystem.api.e2e.test.ts`. See [Real adapter e2e](/docs/reference/testing/real-adapters).
