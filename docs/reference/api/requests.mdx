...
Outbound webhook retries append `webhook_delivery_attempted` events. The payload reuses the notification event id and delivery-attempt id so operators can see retries without polling SQL.
