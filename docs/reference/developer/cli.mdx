---
title: "DSAR CLI"
description: "@dsar/cli provides a command surface that maps to DSAR backend HTTP endpoints for agent and script usage."
group: reference-developer
---

<include src="../../_shared/alpha-warning.mdx" />

`@dsar/cli` provides a command surface that maps to DSAR backend HTTP endpoints for agent and script usage.

## Invocation

Local workspace:

```sh
dsar --help
```

Command help:

```sh
dsar requests get --help
```

Interactive wizard:

```sh
dsar
```

When launched without command args, the CLI enters interactive mode and guides:

1. API URL/output/token setup
2. command domain + command selection
3. required params/body fields
4. execute/edit/cancel confirmation

Published package flow:

```sh
npx dsar --help
```

## Environment and auth

- `DSAR_API_URL` (required unless overridden by `--api-url`)
- `DSAR_API_TOKEN` (optional bearer token)

Global flags:

- `--api-url`, `--token`, `--idempotency-key`, `--output`, `--json`

## Diagnostics

`dsar doctor` runs preflight diagnostics for the CLI/runtime connection:

- API URL config and URL validity
- public runtime status reachability
- authenticated request reachability when `--token` or `DSAR_API_TOKEN` is set
- runtime database read operations through the authenticated request probe
- persistence reachability and schema migration freshness through
  `GET /status/diagnostics`
- registered storage, inbound, and notification adapter health through
  `GET /status/diagnostics`

When the diagnostics endpoint is unavailable on an older backend, `doctor`
keeps the migration check as a warning and adapter health as skipped. Current
backends fail `doctor` when migrations are stale or any registered adapter is
down, and warn when adapters are degraded.

Use `--output json` when you need the structured diagnostic envelope for
automation or CI logs.

```sh
dsar doctor --api-url http://localhost:4086 --token "$DSAR_API_TOKEN"
```

## CLI ↔ API mapping highlights

- `init` -> `POST /init`
- `status` -> `GET /status`
- `subjects get <subjectId>` -> `GET /subjects/{subjectId}`
- `policies list` -> `GET /policies`
- `policies custom register|activate|deactivate` -> custom policy endpoints
- `webhooks inbound resend` -> `POST /webhooks/inbound/resend`
- `webhooks list|replay|replay-all|dlq|tail ...` -> outbound webhook dispatch inspection, replay, dead-letter recovery, and polling
- `requests create|capture` -> `POST /requests`, `POST /requests/capture`
- `requests clock explain <id>` -> `GET /requests/{id}/clock/explain`
- `requests verification ...` -> verification endpoints
- `requests fulfilment create <id>` -> `POST /requests/{id}/fulfilment`
- `requests delivery ...` -> delivery endpoints
- `requests manifest ...` -> manifest endpoints
- `requests appeals ...` -> appeals endpoints
- `requests notifications list|replay ...` -> notification history and replay
- `tenants retention get|put <tenantId>` -> retention endpoints
- `requests audit export|verify <id>` -> audit endpoints

Full parity route map lives in:

- `packages/cli/src/parity/route-map.ts`

## JSON output contract

When `--output json` is used:

- success: `{ ok: true, data, meta: { command } }`
- failure: `{ ok: false, error: { code, message }, meta? }`
- diagnostic failure: `{ ok: false, data, meta: { command } }`

Interactive mode still uses the same final output envelope after command execution.

## Exit codes

- `0`: command completed successfully. `doctor` can still include warnings for degraded adapters or skipped checks when optional diagnostics are unavailable.
- `1`: command failed, an unknown command was requested, or `doctor` found one or more failed checks.

## Notification replay

Notification replay is part of the current CLI surface:

- `dsar requests notifications list <request-id>`
- `dsar requests notifications replay <request-id> <event-id>`

Outbound webhook dispatch replay is available through:

- `dsar webhooks list --status failed`
- `dsar webhooks replay <dispatch-id> --idempotency-key replay-1`
- `dsar webhooks replay-all --status failed --endpoint-id default --limit 100 --idempotency-key replay-all-1`
- `dsar webhooks dlq list`
- `dsar webhooks dlq replay <dispatch-id> --idempotency-key replay-dead-1`
- `dsar webhooks dlq replay-all --idempotency-key replay-dead-all-1`
- `dsar webhooks tail --status failed --endpoint-id default --interval 2000`

`replay` targets one failed or dead webhook dispatch. `replay-all` maps to
`POST /webhooks/dispatches/replay` and builds the bulk replay body from
`--status`, `--endpoint-id`, `--created-after`, `--created-before`, and
`--limit`. `--status` may be `failed` or `dead`. `--idempotency-key` is
required for both replay commands.

`dlq list` is `GET /webhooks/dispatches?status=dead`. `dlq replay` reuses
single-dispatch replay. `dlq replay-all` is bulk replay pinned to `dead`.

`tail` polls `GET /webhooks/dispatches` and streams newly observed dispatches.
It accepts `--status`, `--endpoint-id`, `--interval`, `--limit`, and `--once`.

## Parity policy

Any new backend OpenAPI path + method pair must include:

1. route parity entry in `packages/cli/src/parity/route-map.ts`
2. callable CLI command mapping
3. passing parity test
