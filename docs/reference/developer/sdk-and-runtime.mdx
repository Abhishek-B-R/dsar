...
Use the framework adapter subpath for your server. Each middleware verifies the
`x-dsar-signature` header before it decodes and dispatches the event.

```ts
import { nextWebhookMiddleware } from "@dsar/node-sdk/webhooks/next";

const signingSecret = process.env.DSAR_WEBHOOK_SECRET;
if (!signingSecret) {
...
export const POST = nextWebhookMiddleware({
	handlers: {
		request_captured(event) {
			console.info("Captured DSAR request", event.requestId);
		},
	},
	signingSecret,
});
...
The Express, Hono, and Next.js adapters are exported from
`@dsar/node-sdk/webhooks/express`, `@dsar/node-sdk/webhooks/hono`, and
`@dsar/node-sdk/webhooks/next`. Their `*WebhookHandler` and
`*WebhookMiddleware` names are aliases. When using Express, configure
`express.raw({ type: "application/json" })` so signature verification receives
the original request bytes.

For framework-neutral dispatch, call `client.webhooks.receiver(options)` or
`createWebhookReceiver(options)` from `@dsar/node-sdk/webhooks`. Both accept the
same `handlers` map, and the returned receiver also supports `.on(eventType,
handler)` registration.

The receiver and framework adapters take exactly one `signingSecret`. After
`client.webhooks.rotateKey`, dispatch signs with the new primary immediately, so
a single-secret receiver returns 401 until you deploy the new secret. During the
grace window, verify with `verifyWebhook({ body, signature, keyId, secrets })`
from `@dsar/node-sdk/webhooks`, passing every active secret and the
`x-dsar-signature-key-id` header.
