...
The quickest HTTP surface is kitchen-sink. For a UI, run the subject portal
(in-process SQLite) or point `@dsar/react` at a hosted project on inth.com.

```sh
bunx turbo run dev --filter=./examples/kitchen-sink
bunx turbo run dev --filter=./examples/subject-portal --filter=./examples/dashboard
```
...
- Use [Integration Guides](../integrations/index.md) to wire auth, storage, and
