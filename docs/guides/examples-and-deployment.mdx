...
| `examples/nextjs-deletion-webhook`   | Next.js App Router receiver for signed outbound DSAR webhooks                 | Copy this when you need to erase a local row on fulfil |
| `examples/dashboard`                 | Operator queue using `@dsar/react` and in-process SQLite                      | Local operator UI                                      |
| `examples/subject-portal`            | Subject portal using `@dsar/react` and in-process SQLite                      | File and list requests in the browser                  |
