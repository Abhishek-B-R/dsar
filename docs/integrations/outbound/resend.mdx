...
- Delivery outcomes are persisted as `pending`, `delivered`, `failed`,
  `skipped`, or `dead`.
...
- The first webhook send is written as a `pending` attempt before the HTTP call.
  Remaining retries are scheduled on that row's `next_attempt_at` using 1m, 5m,
  30m, 2h, 6h, then 24h. A worker claims due rows after the originating request
  returns, so a process crash can resume work.
- `retryMaxAttempts` caps the number of webhook sends. Exhausted or
  non-retriable webhook attempts are marked `dead`. Email still uses
  `retryDelayMs` for in-process retries.
...
- List exhausted webhook attempts with `GET /webhooks/dispatches?status=dead`.
  Dead rows are replayable the same way failed rows are.
