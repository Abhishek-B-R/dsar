...
import { E2E_RUNTIME_AUTH, getRelevantOutput, runE2eCli } from "./harness";

const makeWebhookAdapter = (sent: unknown[]): NotificationAdapterContract => ({
...
describe("cLI webhook DLQ", () => {
	it("lists and replays a dead dispatch", async () => {
		const persistence = makeMinimalPersistenceSync();
		await seedDeadDispatch(persistence);
		const sent: unknown[] = [];
		const runtime = dsarInstance({
			adapters: {
				notifications: makeWebhookAdapter(sent),
			},
			config: E2E_RUNTIME_AUTH.config,
			repos: { persistence },
		});
		...
		const listedAfterReplay = await runE2eCli({
			argv: ["webhooks", "dlq", "list"],
			fetch: fetchImpl,
		});
		expect(listedAfterReplay.exitCode).toBe(0);
		expect(getRelevantOutput(listedAfterReplay, 0)).toContain('"total":0');
	});
});
